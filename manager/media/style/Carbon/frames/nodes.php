...
if(IN_MANAGER_MODE!="true") die("<b>INCLUDE_ORDERING_ERROR</b><br /><br />Please use the MODX Content Manager instead of accessing this file directly.");
    ...
    $docgrp = (isset($_SESSION['mgrDocgroups']) && is_array($_SESSION['mgrDocgroups'])) ? implode(",",$_SESSION['mgrDocgroups']) : '';
        ...
        if($modx->db->getRecordCount($result)==0) {
            $output .= '<span class="emptyNode">'.$_lang['empty_folder'].'</span>';
        }
        ...
        while(list($id,$pagetitle,$menutitle,$parent,$isfolder,$published,$deleted,$type,$template,$menuindex,$donthit,$hidemenu,$alias,$contenttype,$privateweb,$privatemgr,$hasAccess) = $modx->db->getRow($result,'num'))
        {
            ...
            $weblinkDisplay = $type=="reference" ? '&nbsp;'.$_style["tree_linkgo"] : '' ;
            $pageIdDisplay = '<i class="current-doc-id">'.($modx_textdir ? '&rlm;':'').$id.'</i>';
            $url = $modx->makeUrl($id);
            ...
            if ($replace =='') {
                if ($donthit) {
                $icon = ($privateweb||$privatemgr) ? $_style["tree_folder_secure_fnt"] : $_style["tree_folder_fnt"];
                ...
                onclick="showPopup('.$id.',\''.addslashes($nodetitle).'\',event);return false;" 
                oncontextmenu="this.onclick(event);return false;" 
                onmouseover="setCNS(this, 1)" onmouseout="setCNS(this, 0)" 
                onmousedown="itemToChange='.$id.'; selectedObjectName=\''.addslashes($nodetitle).'\'; selectedObjectDeleted='.$deleted.'; selectedObjectUrl=\''.$url.'\'"
                >'.$icon.'</i>';

                $output .= '
                <span p="'.$parent.'" 
                class="treeNode" 
                onclick="treeAction('.$id.', \''.addslashes($nodetitle).'\',0); setSelected(this);" 
                ...
                title="'.addslashes($alt).'">
                '.$nodetitleDisplay.$weblinkDisplay.'
                </span>
                '.$pageIdDisplay.'
                </div>';
                ...
                switch (true) {
                    case ($id == $modx->config['site_start']):
                        $icon = $_style["tree_page_home"];
                    break;
                    case ($id == $modx->config['error_page']):
                        $icon = $_style["tree_page_404"];
                    break;
                    case ($id == $modx->config['site_unavailable_page']):
                        $icon = $_style["tree_page_hourglass"];
                    break;
                    case ($id == $modx->config['unauthorized_page']):
                        $icon = $_style["tree_page_info"];
                    break;
                }

                $node .= 
                    '<div class="current-node-element" 
                    id="node'.$id.'" 
                    p="'.$parent.'" 
                    style="white-space: nowrap;">
                    ...
                    onmousedown="itemToChange='.$id.'; selectedObjectName=\''.addslashes($nodetitle).'\'; selectedObjectDeleted='.$deleted.'; selectedObjectUrl=\''.$url.'\'">'.$icon.'</i>
                    ';
                ...
                $node .= 
                    ...
                    p="'.$parent.'" 
                    class="treeNode" 
                    onclick="treeAction('.$id.', \''.addslashes($nodetitle).'\'); setSelected(this);" 
                    onmouseover="setHoverClass(this, 1);" 
                    onmouseout="setHoverClass(this, 0);" 
                    onmousedown="itemToChange='.$id.'; selectedObjectName=\''.addslashes($nodetitle).'\'; selectedObjectDeleted='.$deleted.'; selectedObjectUrl=\''.$url.'\';"
                    oncontextmenu="document.getElementById(\'p'.$id.'\').onclick(event);return false;" 
                ...
                $node .= '
                    ...
                    onmouseout="setCNS(this, 0)" 
                    onmousedown="itemToChange='.$id.'; selectedObjectName=\''.addslashes($nodetitle).'\'; selectedObjectDeleted='.$deleted.'; selectedObjectUrl=\''.$url.'\';" 
                    >'.(($privateweb == 1 || $privatemgr == 1) ? $_style["tree_folderopen_secure_fnt"] : $_style["tree_folderopen_fnt"]).'</i>

                    <i class="folder-toggle-indicator" 
                    id="s'.$id.'"
                    onclick="toggleNode(this,'.($indent+1).','.$id.','.$expandAll.','. (($privateweb == 1 || $privatemgr == 1) ? '1' : '0') .'); return false;" 
                    oncontextmenu="this.onclick(event); return false;" 
                    >'.$_style["tree_minusnode"].'</i>';
                ...
                $node .= '
                    ...
                    title="'.addslashes($alt).'">'.$nodetitleDisplay.$weblinkDisplay.'</span>'.$pageIdDisplay.'<div style="display:block">';
                ...
                $node .= '
                    ...
                    onmouseout="setCNS(this, 0)" 
                    onmousedown="itemToChange='.$id.'; selectedObjectName=\''.addslashes($nodetitle).'\'; selectedObjectDeleted='.$deleted.'; selectedObjectUrl=\''.$url.'\';" 
                    >'.(($privateweb == 1 || $privatemgr == 1) ? $_style["tree_folder_secure_fnt"] : $_style["tree_folder_fnt"]).'</i>

                    <i class="folder-toggle-indicator" 
                    id="s'.$id.'" 
                    onclick="toggleNode(this,'.($indent+1).','.$id.','.$expandAll.','. (($privateweb == 1 || $privatemgr == 1) ? '1' : '0') .'); return false;" 
                    oncontextmenu="this.onclick(event); return false;" 
                    >'.$_style["tree_plusnode"].'</i>';
                ...
                $node .= '
                    ...
                    onmouseover="setHoverClass(this, 1);" 
                    onmouseout="setHoverClass(this, 0);" 
                    onmousedown="itemToChange='.$id.'; selectedObjectName=\''.addslashes($nodetitle).'\'; selectedObjectDeleted='.$deleted.'; selectedObjectUrl=\''.$url.'\';"oncontextmenu="document.getElementById(\'f'.$id.'\').onclick(event);return false;" 
                    title="'.addslashes($alt).'">'.$nodetitleDisplay.$weblinkDisplay.'</span>'.$pageIdDisplay.'<div style="display:none"></div></div>';
