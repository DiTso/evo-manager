...
if(IN_MANAGER_MODE!="true") die("<b>INCLUDE_ORDERING_ERROR</b><br /><br />Please use the MODX Content Manager instead of accessing this file directly.");
    ...
    function constructLink($action, $img, $text, $allowed) {
        if($allowed==1) { ?>
            <div class="menuLink" onclick="document.getElementById('treeiframe').contentWindow.menuHandler(<?php echo $action ; ?>); document.getElementById('treeiframe').contentWindow.hideMenu();">
        <?php } else { ?>
...
<head>
    ...
    function setActiveFromContextMenu( doc_id ){
        if ($$('#node'+doc_id+' span')[0].getAttribute("class") == "treeNodeSelected") {
            hideMenu(1);
        } else {
            $$('.treeNodeSelected').removeClass('treeNodeSelected');
            $$('#node'+doc_id+' span')[0].setAttribute("class", "treeNodeSelected");
        }
    }
    // Context menu stuff
    ...
    function hideMenu() {
        ...
        parent.document.getElementById('mx_contextmenu').style.visibility = 'hidden';
    }
    </script>
...
?>
    ...
    function resizeTree() {
        ...
        var tree = $('treeHolder');
        var tmnu = $('treeMenu');
        tree.style.width = (win['width'])+'px';
        tree.style.height = (win['height']-tree.offsetTop)+'px';
    ...
    function showPopup(id,title,e){
        //parent.document.getElementById('pupup-context-placeholder').innerHTML = '';
        var x, y;
        if ($('mx_contextmenu')) {
            var mnu = $('mx_contextmenu');
        } else {
            var mnu = parent.document.getElementById('mx_contextmenu');
        }
        //var mnu = $('mx_contextmenu');
        mnu.setAttribute("dataid", id);
        
        var bodyHeight = parseInt(document.body.offsetHeight);
        x = e.clientX+5;
        y = e.clientY;
        y = y+100;  //mnu.offsetHeight/2;
        yp = y+10-mnu.offsetHeight;
        var itemToChange=id;
    ...
    function dopopup(x,y) {
        ...
        if ($('mx_contextmenu')) {
            ...
            var h,context = parent.document.getElementById('mx_contextmenu');
            var elm = parent.document.getElementById("nameHolder");
        }
        context.style.left= x<?php echo $modx_textdir ? '-190' : '';?>+"px"; //offset menu to the left if rtl is selected
        if (y < parent.document.getElementById('manager').offsetHeight/2 ) {
            context.style.top = y+"px";
        } else {
            context.style.top = yp+"px";
        }
        
        elm.innerHTML = selectedObjectName+" ("+headingID+")";
        
        context.style.visibility = 'visible';
        context.style.display = 'block';
        // NEW popup to main body
        parent.document.getElementById('pupup-context-placeholder').appendChild(context);
    ...
    function toggleNode(node,indent,parent,expandAll,privatenode) {
        ...
        var sign = document.getElementById("s"+parent);
        var folder = document.getElementById("f"+parent);
        
        if (rpcNode.style.display != 'block') {
            // expand
            if(sign && sign.innerHTML.indexOf('<?php echo $_style['tree_plusnode']?>')>-1) {
                sign.innerHTML = '<?php echo $_style["tree_minusnode"]; ?>';
                folder.innerHTML = (privatenode == '0') ? '<?php echo $_style["tree_folderopen_fnt"]; ?>' :'<?php echo $_style["tree_folderopen_secure_fnt"]; ?>';
            }
            ...
            if (rpcNodeText=="" || rpcNodeText.indexOf(loadText)>0) {
                ...
                rpcNode.innerHTML = "<span class='emptyNode' style='white-space:nowrap;'>"+loadText+"...<\/span>";
            ...
            // collapse
            if(sign && sign.innerHTML.indexOf('<?php echo $_style["tree_minusnode"]; ?>')>-1) {
                sign.innerHTML = '<?php echo $_style["tree_plusnode"]; ?>';
                folder.innerHTML = (privatenode == '0') ? '<?php echo $_style["tree_folder_fnt"]; ?>' : '<?php echo $_style["tree_folder_secure_fnt"]; ?>';
            }
            //rpcNode.innerHTML = '';
    ...
    function showSorter() {
        if(currSorterState=="none") {
            ...
            document.getElementById('Button5').setAttribute('class', 'treeButton active');
            document.getElementById('floater').style.display=currSorterState;
        } else {
            document.getElementById('Button5').setAttribute('class', 'treeButton');
            currSorterState="none";
            document.getElementById('floater').style.display=currSorterState;
        }
    }
...
<body id="tree-frmae-body" oncontextmenu="return false;" onClick="hideMenu(1);" class="<?php echo $modx_textdir ? ' rtl':''?>">
  ...
  <tr>
    <td>
        <table cellpadding="0" cellspacing="0" border="0">
            ...
            <?php } ?>
			 <td><a href="#" class="treeButton" title="Управление элементами" onclick="window.open('index.php?a=76','gener','width=800,height=600,top='+((screen.height-600)/2)+',left='+((screen.width-800)/2)+',toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no')"><?php echo $_style['elements_icon']; ?></a></td>
            </tr>
        </table>
    </td>
  </tr>
...
<?php
    ...
    if (is_array($evtOut))
        echo implode("\n", $evtOut);
?>
    <div><span class="rootNode" onClick="treeAction(0, '<?php echo addslashes($site_name); ?>');"><?php echo $_style['tree_showtree']; ?>&nbsp;<b><?php echo $site_name; ?></b></span><div id="treeRoot"></div></div>
<?php
    // invoke OnTreeRender event
...
<div id="mx_contextmenu" onselectstart="return false;" oncontextmenu="return false;">
